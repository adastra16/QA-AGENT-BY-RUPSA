...
Short description
-----------------
QA-Agent generates structured testcases from uploaded documents and optionally produces Selenium test scripts for browser automation.

Highlights
----------
- FastAPI backend for API endpoints
- Streamlit front-end for uploads and interactions
- ChromaDB & sentence-transformers for semantic search
- Selenium script generator for quick browser tests

Requirements
------------
- Python 3.10+ (recommended)
- pip
- For Selenium tests: appropriate WebDriver (chromedriver or geckodriver)
...
2. Run backend (default port 8000):
...
4. Open the UI at: http://localhost:8501 and API docs at: http://127.0.0.1:8000/docs
...
```
QA-AGENT/
├── agents/                # automation agent scripts
├── assets/                # reference docs used by the app
├── backend/               # FastAPI backend and APIs
├── chroma_db/             # ChromaDB persisted files
├── ingest/                # ingestion and embedding helpers
├── ui/                    # Streamlit front-end
├── selenium_debug/        # debugging helpers for Selenium
├── selenium_scripts/      # example Selenium scripts
├── uploaded_assets/       # uploaded files via UI
├── uploaded_docs/         # sample uploaded docs
...
Deployment (Render)
-------------------
- Use a Web Service on Render and set the Start Command to:

```bash
uvicorn backend.main:app --host 0.0.0.0 --port $PORT --log-level info
```

- Use `/health` as the health-check path. If you are using `python backend/run.py`, the script already respects `$PORT`.

Common troubleshooting
----------------------
- If Render reports "No open ports detected": make sure the start command uses `$PORT` and the service type is Web Service. If your start command is `python backend/run.py`, ensure that environment injection sets `$PORT`.
- If the app takes time to load (embedding model loading etc.), increase the service start timeout.

Contributing
------------
- Fork, add tests, and open a PR. Keep PR descriptions concise.

License and Contact
-------------------
Educational project. Contact Rupsa Nanda for questions or clarifications.
# QA-Agent — Automated Testcase & Selenium Script Generator
